class Api::CardPurchasesController < ApplicationController
  ...
  def proccess_order_and_return_response(params)
      user = User.find(params[:user_id].to_i) if User.exists?(params[:user_id].to_i)
      ...
      if user
        ...
        begin
          if order.save
            return Api::SuccessResponse.new(order)
          else
        ...
        return Api::ErrorResponse.user_doesnt_exist_into_our_database
      end
  end
end
